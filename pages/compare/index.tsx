...
export default function Compare({ $productName }: CompareProps) {
  ...
  // const handleClickSearch2 = (e: React.MouseEvent<HTMLInputElement>) => {
  //   const productName = e.currentTarget.innerText;
  ...
  // const { data: similarProducts2, isSuccess } = useQuery({
  ...
  // const [productTwoData, setProductTwoData] = useState<any>();
    ...
    <>
      <BodyContainer>
        ...
        {/* <InputContainer>
          ...
          {isShow2 && (
            <ProductCheckedNameWrap>
              {similarProducts2?.list?.map((product) => (
                <ProductCheckedNameItem key={product.id} onClick={handleClickSearch2}>
                  {product.name}
                </ProductCheckedNameItem>
              ))}
            </ProductCheckedNameWrap>
        ...
        <StyledPrimaryButton type="button" onClick={handleSubmit}>
          비교하기
        </StyledPrimaryButton>
      </BodyContainer>
      {/* <Loading /> */}
